...
let sgd ?(lr = 0.001) f =
  ...
  return Var.List.[loss; [float_map2 (fun p g -> p -$ (g *$. lr)) params grad]]
...
let adamw ?(lr = 0.001) ?(betas = (0.9, 0.999)) ?(eps = 1e-8)
  ...
  let* (E t) = new_param (E (Tensor.scalar F32 1.)) in
  let* m = new_param (Runtime.HostValue.zeros (Var.value_types params)) in
  let* v = new_param (Runtime.HostValue.zeros (Var.value_types params)) in
  ...
  let t = t +$. 1. in
  return Var.List.[loss; [params; E t; m; v]]

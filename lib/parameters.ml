type ('a, 'b) t = ('a Var.t -> 'b) * 'a Runtime.HostValue.t
...
let bind : type a b c d.
 ...
 fun (f, params) f' ->
  ( (fun (x :: xs) ->
      ...
      f' xs )
  , let dummy_params =
      Var.arg_of_value_type @@ Runtime.HostValue.value_type params
    in
...
let new_param : type a. a Runtime.HostValue.t -> (a, a Var.t) t =
 fun t -> ((fun x -> x), t)

let to_fun : type a b. (a, b) t -> a Var.t -> b = fst

let initial : type a b c.
    a Value_type.t -> (a Var.t -> (b, c) t) -> b Runtime.HostValue.t =
 fun t f ->
  let dummy_x = Var.arg_of_value_type t in
  Effects.dummy_handler @@ fun () -> snd @@ f dummy_x

let params_for : type a b. (a, b) t -> (a, a Var.t) t =

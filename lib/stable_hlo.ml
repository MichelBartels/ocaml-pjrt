...
let rec op_to_string (op : op) =
  ...
  | Some op_name ->
      ...
      outputs ^ "stablehlo.reduce(%" ^ data_input ^ " init: %" ^ init_input
      ^ ") applies " ^ op_name ^ " across dimensions = " ^ dims ^ " : "
      ^ signature
  | None ->
...
let%expect_test "op_to_string" =
  ...
  [%expect
    {| %output = "stablehlo.add"(%input) : (tensor<2x3xf32>) -> (tensor<2x3xf32>) |}] ;
  let op_with_attrs =
    { inputs= [("input", Tensor_type ([2; 3], F32))]
    ; outputs= [("output", Tensor_type ([2; 3], F32))]
  ...
  let op_with_reduce =
    { inputs=
        [("data", Tensor_type ([2; 3], F32)); ("init", Tensor_type ([], F32))]
  ...
  [%expect
    {| %output = stablehlo.reduce(%data init: %init) applies stablehlo.add across dimensions = [0] : (tensor<2x3xf32>, tensor<f32>) -> (tensor<3xf32>) |}]

let%expect_test "func_to_anonymous_string" =
  let func =
    { id= "add"
    ; inputs=
        [("x", Tensor_type ([2; 3], F32)); ("y", Tensor_type ([2; 3], F32))]
    ; outputs= [Tensor_type ([2; 3], F32)]
    ; body=
        [ { inputs=
              [ ("x", Tensor_type ([2; 3], F32))
              ; ("y", Tensor_type ([2; 3], F32)) ]
          ; outputs= [("output", Tensor_type ([2; 3], F32))]
          ; name= "stablehlo.add"
          ; attributes= []
          ; anonymous_functions= []
